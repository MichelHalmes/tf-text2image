...
def get_model(encoder):
    ...
    layers.add(K.layers.Embedding(input_dim=encoder.vocab_size, output_dim=4))
    layers.add(K.layers.GRU(16))

    FEAT_HW = 8
    FEAT_C = 8
    layers.add(K.layers.Dense(FEAT_HW*FEAT_HW*FEAT_C, activation='relu'))
    layers.add(K.layers.Reshape((FEAT_HW, FEAT_HW, FEAT_C)))

    layers.add(K.layers.Conv2DTranspose(filters=6, kernel_size=3, padding="same", strides=2, activation="relu"))
    layers.add(K.layers.Conv2DTranspose(filters=6, kernel_size=3, padding="same", strides=2, activation="relu"))
    layers.add(K.layers.Conv2DTranspose(filters=4, kernel_size=3, padding="same", strides=2, activation="relu"))
    layers.add(K.layers.Conv2DTranspose(filters=4, kernel_size=3, padding="same", strides=2, activation="relu"))
    layers.add(K.layers.Conv2D(filters=3, kernel_size=2, padding="same", activation="relu"))
